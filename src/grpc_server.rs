use crate::market_manager::{MarketManager, OrderbookSnapshot};
use crate::fast_orderbook::{FastOrderbook, OrderbookDelta};
...
pub struct OrderbookServiceImpl {
    market_manager: Arc<RwLock<MarketManager>>,
    update_rx: Arc<RwLock<broadcast::Receiver<OrderbookSnapshot>>>,
}

impl OrderbookServiceImpl {
    pub fn new(
        market_manager: Arc<RwLock<MarketManager>>,
        update_rx: broadcast::Receiver<OrderbookSnapshot>,
    ) -> Self {
        Self {
            market_manager,
...
impl OrderbookService for OrderbookServiceImpl {
    ...
    async fn subscribe_orderbook(
        ...
        info!("New orderbook subscription for markets: {:?}", requested_markets);
        ...
        tokio::spawn(async move {
            ...
            let snapshots_to_send = {
                let manager = market_manager.read();
                requested_markets
                    .iter()
                    .filter_map(|market_id| {
                        manager
                            .get_orderbook_snapshot(*market_id, 50)
                            .map(convert_to_pb_snapshot)
                    })
                    .collect::<Vec<_>>()
            };
            ...
            while let Ok(snapshot) = rx.recv().await {
                if requested_markets.contains(&snapshot.market_id) {
                    if tx.send(Ok(convert_to_pb_snapshot(snapshot))).await.is_err() {
                        break;
                    }
                }
            }
    ...
    async fn get_orderbook(
        ...
        let manager = self.market_manager.read();
        match manager.get_orderbook_snapshot(req.market_id, depth) {
            Some(snapshot) => Ok(Response::new(convert_to_pb_snapshot(snapshot))),
    ...
    async fn get_markets(
        ...
        let markets = manager
            .get_market_info()
            .into_iter()
            .map(|(market_id, symbol)| MarketInfo {
                market_id,
                symbol,
                active: true,
            })
...
fn convert_to_pb_snapshot(snapshot: OrderbookSnapshot) -> PbOrderbookSnapshot {
    PbOrderbookSnapshot {
        market_id: snapshot.market_id,
        symbol: snapshot.symbol,
        timestamp_us: snapshot.timestamp,
        ...
        bids: snapshot
            ...
            .map(|(price, quantity)| PriceLevel {
                price,
                quantity,
                order_count: 0, // We'll update this later if needed
            })
            .collect(),
        asks: snapshot
            .asks
...
pub fn create_grpc_server(
    market_manager: Arc<RwLock<MarketManager>>,
    update_rx: broadcast::Receiver<OrderbookSnapshot>,
) -> OrderbookServiceServer<OrderbookServiceImpl> {
    let service = OrderbookServiceImpl::new(market_manager, update_rx);
